...
(function (({
 ...
 document.dispatchEvent(new CustomEvent("_check-for-updates", { detail: { ...SCRIPT_DATA } })) } function isOutdatedScript(a, b({
 ...
 return authURL ? request({ url: authURL, responseType: "text" }) : Promise.reject() } authURL && requestAuthData().then(a => { sessionStorage.setItem("gp_auth-data", a) }).catch(a => { });
    ...
    class LocalizationController extends EventTarget {
        ...
        constructor(({
            ...
            b({
 document.dispatchEvent(new CustomEvent("_gp_l10n", { detail: { lang: a, entries: b } })) } getLocalization(({
 return { lang, entries } = this } getCache(({
 return JSON.parse(localStorage.getItem(LocalizationController.STORAGE)) ?? {} } setCache(a, b, c({
 localStorage.setItem(LocalizationController.STORAGE, JSON.stringify({ lang: a, hash: b, entries: c })) } load(a({
 return request({ url: `${LocalizationController.PATH}/locales/${a}.json` }) } loadHashes(({
 ...
 return a.replace(/(_|\*|`|~|\\)/g, "\\$1") } function capitalizeString(a({
 ...
 return a === REQUEST_CACHE.DEFAULT ? {} : { "Cache-Control": a } } console.log("gpmod: extensions");
