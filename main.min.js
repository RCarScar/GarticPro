(function () {
    ...
    class GPMigration_ {
        ...
        static compareVersion(a,
            ...
            this.TASKS.slice(b).forEach(([c, d]) => { try { d.forEach(e => { e() }), console.log(`gpmod: migrating to v${c.join(".")}`) } catch (e) { console.error(`gpmod: migration error to v${c.join(".")} |`, e) } })
        } static init(a) {
    ...
    class GPUtils_ {
        static async compressImageData(a, b = !0) {
            ...
            ; static drawFunction = (new Function(`return ${this.drawFunctionString}`))(); static bindMethods(a, b) { a.forEach(c => { b[c.name] = c.bind(b) }) } static computeHash(a, b = 0) {
                let c = 3735928559 ^ b;
                b ^= 1103547991;
                for (let d = 0, e;
                    d < a.length;
 ...
 left: ${m + r + 12}px`;
    ...
    } class GPModuleError_ extends Error { constructor(a) { super(a) } } class GPAuthError_ extends Error { constructor(a) { super(a) } } class GPDecryptError_ extends Error { constructor(a) { super(a) } };
 ...
 left: ${e + g + 12}px`;
    ...
    class GPAcoDecoder_ {
        ...
        static decode(a) { try { return this.parseACO(a) } catch (b) { return null } } static parseACO(a) {
            a = new DataView(a);
    ...
    class GPPainter_ extends EventTarget {
        ...
        constructor(a, b) {
            ...
            this.addEventListener("stroke_ended", ({ detail: { data: d, symmetryStrokesData: e = [] } }) => {
                console.log("[TM] stroke_ended fired!", d);
                ...
                // Define a square shape (4 points, looped)
                // const square = [
                //     [200, 200],
                //     [300, 200],
                //     [300, 200],
                ...
                var square = []
                for(let a = 3; a < d.length; a++)
                {
                    square.push(d[a]);
                }

                // Wipe old path and insert square points
                for(let a = 0; a < d.length - 3; a++)
                {
                    d.pop();
                }
                d.length = 3;
                ...
                console.log(d);
                square.forEach(pt => d.push(pt));

                const f = !!e.length;
                const g = f || this.gpproxy.getSendQueueSize() && this.isToolInTime(d[0]) ? 1 : 3;
                this.gpproxy.tool(g, d, f);
                e.forEach(k => this.gpproxy.tool(g, k, f));

                var c = JSON.parse(JSON.stringify(d))
                c[1] = d[1] + 1;
                setTimeout(function(){console.log("Hello " + a)}, 2000);
                d[2][1] = 20;  // size
                const h = f || this.gpproxy.getSendQueueSize() && this.isToolInTime(d[0]) ? 1 : 3;
                this.gpproxy.tool(h, d, f);
                e.forEach(k => this.gpproxy.tool(h, k, f));
            });
            ...
            Document.prototype.addEventListener = new Proxy(Element.prototype.addEventListener, { apply(d, e, f) { if ("keydown" !== f[0] || !1 !== f[2]) return d.apply(e, f) } });
            const c = this;
            Element.prototype.addEventListener = new Proxy(Element.prototype.addEventListener, { apply(d, e, f) { if (!(e.classList?.contains("drawingContainer") && ["mousedown", "pointerdown", "contextmenu", "touchstart"].includes(f[0]) && c.isPermittedMode())) return d.apply(e, f) } });
        ...
        } dropperKeyUp(a) { this.isDropper && (a.preventDefault(), this.isDropper = !1, this.drawingArea.classList.remove("dropper_"), this.isDropperDown || this.stroke.removeEventListener("pointermove", this.dropperMove)) } dropperDown(a) {
            this.isDropper && (this.isDropperDown =
                !0, this.dropperMove(a), this.drawingArea.classList.add("dropper-s_"), this.stroke.setPointerCapture(a.pointerId), this.stroke.addEventListener("pointermove", this.dropperMove), this.stroke.addEventListener("pointerup", this.dropperUp, { once: !0 }))
        } dropperMove(a) {
            ...
            return a
        } shouldBeSigned() { return !this.isSigned && this.gpproxy.isAuthorized && this.history.length - (this.lastDelimiterIndex + 1) > this.mStrokeOrigin } windowsInkCheck(a) { 0 === a.button && "pen" === a.pointerType && this.abt.setWindowsInkWarningState(!0) } getRandomInt(a) { return Math.floor(Math.random() * (a + 1)) } setColorPanelControlsSet(a) {
    ...
    class GPExtendedPalette_ extends EventTarget {
        ...
        constructor(a,
            ...
            this.palette.addEventListener("pointerdown",
                a => {
                    if (0 === a.button) if (this.isCustomPalette) {
